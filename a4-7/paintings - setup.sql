...
SELECT
	first_name, last_name, painting_id, title, price
FROM art AS left_table
INNER JOIN
	(	SELECT
			painter_id, MAX(price) AS max_price
		FROM
			art
		GROUP BY painter_id
	) AS right_table 
    ON left_table.painter_id = right_table.painter_id
	AND left_table.price = right_table.max_price
;

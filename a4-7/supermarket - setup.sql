...
SELECT 
    customer_name, item_id, item_name, item_price
FROM
    shopping AS outer_table
INNER JOIN
    (
    SELECT
		customer_id, max(item_price) AS max_price
	FROM
		shopping
	GROUP BY customer_id
    ) AS inner_table
    ON outer_table.customer_id = inner_table.customer_id AND outer_table.item_price = inner_table.max_price
;
